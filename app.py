...
def on_disconnect():
    ...
    connection = connection_list.get(request.sid)
    if not connection:
        return

    if connection["type"] == "host":
        session_list.pop(connection["session"], None)
        emit("host_disconnect", {"session": connection["session"]}, broadcast=True)
    elif connection["type"] == "user":
        session = session_list.get(connection["session"])
        if session and connection["name"] in session.get("users", []):
            session["users"].remove(connection["name"])

    connection_list.pop(request.sid, None)
    print(connection_list)
    print(session_list)
...
def add_host(data):
    ...
    while True:
        session_name = "".join(
            random.SystemRandom().choice(string.ascii_uppercase) for _ in range(5)
        )
        session_check = session_list.get(session_name, -1)
        if session_check == -1:
            break

    connection_list[request.sid] = {
        "type": "host",
...
def join_session(data):
    if data.get("name") in session_list.get(data.get("session"), {}).get("users", []):
        ...
        emit(
            "session_data",
            {"session_data": session_list.get(data.get("session"), -1)},
        )
